			...
			<div v-if="!this.isLoading">
				...
				<div id="ref-result-row" class="container">
					<span v-if="this.countFinancements < 2" class="ref-results-count">
						Résultat (<span class="ref-results-count-number">{{ this.countFinancements }}</span>)
					</span>
					<span v-else class="ref-results-count">
						Résultats (<span class="ref-results-count-number">{{ this.countFinancements }}</span>)
					</span>
					<div id="ref-results-list">
						<ul id="ul-ref-results-list" class="container">
							...
							<span class="col-md-8 col-sm-11">
								<button v-for="tag in financement.tags" :key="tag.id" class="ref-results-tags btn btn-outline-info" href="#" disabled>
									{{ tag }}
								</button>
							</span>
...
<script>
	export default {
		...
		methods: {
			...
			concat: function (intitule,tags) {
				return intitule.concat(tags.join(""));
			}
		},
		data: function(){
			return {
		...
		computed: {
			filteredFinancements() {
				return this.financements.filter(financement => {
					if (this.search != "") {
						return this.concat(financement.intitule, financement.tags).match(this.search)
					}
					return financement.intitule				
				})
			},
			countFinancements() {

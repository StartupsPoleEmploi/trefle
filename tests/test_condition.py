...
def test_gt_int_condition_should_raise_with_invalid_value():
    condition = Condition('age > 18')
    with pytest.raises(ValueError):
        condition.assess(age='notanint')


def test_equal_int_condition_should_raise_with_invalid_value():
    condition = Condition('age = 18')
    with pytest.raises(ValueError):
        condition.assess(age='notanint')


def test_equal_true_condition_should_raise_with_invalid_value():
    condition = Condition('inscrit = OUI')
    with pytest.raises(ValueError):

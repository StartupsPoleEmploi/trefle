...
@pytest.mark.parametrize("insee", ["123456", "A12345"])
...
@pytest.mark.parametrize("search_term", [1147, "cabinets médicaux"])
async def test_idcc_search(client, search_term):
    resp = await client.get("/idcc?q=" + urllib.parse.quote_plus(str(search_term)))
    assert resp.status == HTTPStatus.OK
    assert json.loads(resp.body) == {
        "1147": {
            "convention collective": "Convention collective du personnel des cabinets médicaux (médecin)",
